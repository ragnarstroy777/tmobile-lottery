...
function showPrizeList(currentPrizeIndex) {
  ...
  if (currentPrize.type === defaultType) {
    currentPrize.count === "Без ограничений";
  }
  // Заменяем статусную строку на количество участников (будет установлено из index.js)
  let htmlCode = `<div class="prize-mess"><label id="participantsCount" class="prize-shine">Количество участиников: —</label></div><ul class="prize-list">`;
  prizes.forEach(item => {
...
let setPrizeData = (function () {
  return function (currentPrizeIndex, count, isInit) {
    ...
    // Верхние элементы статуса убраны, поэтому не инициализируем их
    ...
    if (lasetPrizeIndex !== currentPrizeIndex) {
      ...
      // Статусная строка убрана

      lasetPrizeIndex = currentPrizeIndex;
    }

    // Для специального приза отдельной строки статуса больше нет
    ...
    elements.text && (elements.text.textContent = count + "/" + displayTotal);
  };
})();
